...
@LastEditors: steven
@LastEditTime: 2020-05-28 23:36:38
...
#TODO:https://www.kancloud.cn/digest/jackhej/213922
'''
import itertools
...
def twentyfour(cards):
    ...
    for nums in itertools.permutations(cards):  #? 四个数a,b,c,d随机？
        for ops in itertools.product('+-*/', repeat=3):  # 三个运算符（可重复！）
            # 构造三种中缀表达式 (bsd) 中缀表达式不容易被计算机解析，但仍被许多程序语言使用，因为它符合人们的普遍用法。
            ...
            for bds in [bds1, bds2, bds3]:  # 遍历
                try:
                    if abs(eval(bds) - 24.0) < 1e-10:  # ? eval函数:可以将字符串转化成代码并执行
                        # math.isclose()：为什么这里需要使用 math.isclose() ，而不是直接使用==运算符呢？这是因为最后算出来的表达式可能有精度问题，例如23.9...或者24.0...等数字
                        # https://juejin.im/post/5d3e2b91f265da1bd605eceb
                        return bds
