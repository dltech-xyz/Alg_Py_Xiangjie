...
@LastEditors: steven
@LastEditTime: 2020-06-09 11:20:28
@Description:
'''
...
class string(single_list):
    ...
    def gen_next(p):
        j, k, n = 0, -1, p.length() # k 实际上就是pnext[j]
        # p.value[j] 表示前缀， p.value[k]表示后缀。
        pnext = [-1] * n # 主要目的是让第一个元素为-1，可参考： next[0] = -1 next 数组考虑的是除当前字符外的最长相同前缀后缀，所以通过第①步骤求得各个前缀后缀的公共元素的最大长度后，只要稍作变形即可
        while j < n - 1:
            if k == -1 or p.value[j] == p.value[k]:
                j, k = j + 1, k + 1     # 新的元素匹配上了，k即前缀后缀公共元素的最大长度+1。
                if p.value[j] != p.value[k]:
