...
@LastEditors: steven
@LastEditTime: 2020-06-08 22:21:55
@Description:https://blog.csdn.net/TomorrowAndTuture/article/details/89456405
# TODO:https://logikoisto.github.io/2019/linked_list/
'''
...
class LNode(object):
    def __init__(self, elem, next_=None):
        ...
        self.elem = elem
        self.next = next_ # Node的指针部分默认指向None
...
class LList(object):
    ...
    def pop_last(self):
        ...
        if p.next is None:
            ...
            return e
        while p.next.next:
            p = p.next # P是一个指向当前节点的指针，而p->next是指向当前节点的下一个节点的指针。
        e = p.next.elem
        p.next = None
        self._num -= 1
        return e
    ...
    def filter(self, pred):
        ...
        while p: # is not None
            if pred(p.elem):
                yield p.elem # 如果有找到，直接返回，否则遍历下一个节点
            p = p.next
    ...
    def printall(self):
        ...
        while p:
            print(p.elem, end="")
            if p.next:       # 当p还有下一个节点，使用逗号添加元素分隔符
                print(", ",end="")
            p = p.next
        print("")
    ...
    #删除第一个出现的elem
    def remove(self, elem): # https://my.oschina.net/u/4353280/blog/3505389
        p = self._head
        pre = None
        while p:
            # 找到了指定元素
            if p.elem == elem:
                # 如果第一个就是删除的节点
                if not pre: # https://stackoverflow.com/questions/2710940/python-if-x-is-not-none-or-if-not-x-is-none
                    # https://www.cnblogs.com/wangzhao2016/p/6763431.html
                    self._head = p.next # 就是第一个元素时。pre 就是 none
                else:
                    pre.next = p.next  # 将删除位置前一个节点的next指向删除位置的后一个节点.
                break
            else:
                # 继续按链表后移节点
                pre = p
