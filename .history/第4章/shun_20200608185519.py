...
@LastEditors: steven
@LastEditTime: 2020-06-08 18:55:19
@Description:
'''
...
class SeqList(object):
    ...
    def insert(self,key,value):
        ...
        if key >= self.num:
            ...
            #移动key后的元素
            for i in range(self.num, key, -1):# 反向
    ...
    def pop(self,key=-1):
        ...
        elif key == -1:                 # 默认删除最后的元素
            self.num -= 1               # 原来的数还在，但列表不识别他
        else:                           # 删除其他的元素时
            for i in range(key,self.num-1):# key后的元素向前移动
                self.date[i] = self.date[i+1]
            self.num -= 1
    #搜索操作
    ...
    #列表反转
    def reverse(self):
        i,j = 0, self.num -1 # 最后的2，不识别，所以反转，没有问题。。
        while i<j:
